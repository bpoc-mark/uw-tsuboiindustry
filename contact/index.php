...
// エラー表示設定
$temp_err = array(
	'__Error_Input_Data__'		=> '必須項目です',	'__Error_Marge_Data__'		=> '入力された内容に不足があります',	'__Error_Mail_Address__'	=> 'メールアドレスが正しくありません',	'__Error_Mail_Check__'		=> 'メールアドレスが一致しません',	'__Error_Text_Max__'		=> '文字数が多すぎます（__Text_Max__字まで）'
);
...
<p class="btn_back"><input type="submit" value="戻る" class="pushbtn link-btn" onclick="history.back(); return false;" /></p>
EOD;
	} else {
		// 項目が正しい場合のHTML出力
		$submit = <<< EOD
...
<ul class="koumoku cf">
<li class="btn_back left">
<input type="submit" value="戻る" class="pushbtn link-btn" onclick="history.back(); return false;" />
<li class="send right">
<input type="submit" value="送信する" name="submit" id="submit" class="pushbtn link-btn" />
</li>
</li>
</ul>
EOD;
...
// 設定を配列に格納
$sfm_cfg = array(
	'ref_check'			=> $ref_check,	'ill_char'			=> $ill_char,	'ill_slash'			=> $ill_slash,	'use_ssl'			=> $use_ssl,	'mailTo'			=> $mailTo,	'baseEnc'			=> $baseEnc,	'maxText'			=> $maxText,	'mailBcc'			=> $mailBcc,	'replyBcc'			=> $replyBcc,	'replyAddress'		=> $replyAddress,	'replyName'			=> $replyName,	'returnPath'		=> $returnPath,	'temp_html'			=> $temp_html,	'temp_err'			=> $temp_err,	'temp_err_result'	=> $temp_err_result,	'name_marge'		=> $name_marge,	'script_version'	=> $script_version
);
		...
		switch ($mode) {
			...
			case 'CONFIRM':
				...
				foreach ($_POST as $key => $value) {
					...
					} elseif ($cfg['maxText'] && mb_strlen($value) > $cfg['maxText']) {
